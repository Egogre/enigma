...
class DecryptTest < Minitest::Test
  ...
  def test_user_happy_path
    read_file = File.open('./test/encrypted_message.txt')
    write_file = File.new('./test/decrypted_message.txt', 'w')
    key = 12148
    decryptor = Decrypt.new(read_file, write_file, key)
    
    decryptor.decrypt_file
    
    check_file = File.open('./test/decrypted_message.txt')
    check_read = check_file.read
    read_file.rewind
    test_read = read_file.read
    original_file = File.open('./test/sample_message.txt')
